...
tail -n $n_lines /crabprod/CSstoragePath/aperez/out/pool_partition_fresh|grep -v "0 0 0 0"| awk -v var="$ratio" 'NR % var == 0' >/home/aperez/status/input_part_fresh$int
...
tail -n $n_lines /crabprod/CSstoragePath/aperez/out/pool_partition_drain|grep -v "0 0 0 0"|awk -v var="$ratio" 'NR % var == 0' >/home/aperez/status/input_part_retire$int
while read -r line; do
        time=$(echo $line |awk '{print $1}')
        let timemil=1000*$time
        content=$(echo $line |awk '{print $2", "$3", "$4", "$5", "$6", "$7", "$8", "$9}')
