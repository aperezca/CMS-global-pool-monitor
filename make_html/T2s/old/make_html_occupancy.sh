...
OUT="/crabprod/CSstoragePath/aperez/HTML/T2s/"$long"multicore_occupancy_t2s_"$int"h.html"
echo '<html>
<head>
...
for site in `cat /home/aperez/entries/T2_sites`; do
	...
	data_$site.addColumn('number', 'pilot_9');
        data_$site.addColumn('number', 'pilot_10');
        data_$site.addColumn('number', 'pilot_11');
        data_$site.addColumn('number', 'pilot_12');">>$OUT
	fi
	...
	if [ $site == "T1_RU_JINR" ]; then
	echo "colors: ['#FF0000', '#FF2000', '#FF4000', '#FF6000', '#FF8000', '#FFBF00', '#FFFF00', '#80FF00', '#00FF00', '#00FFFF', '#00BFFF', '#0040FF', '#0000FF'],">>$OUT
	else
...
<body>
    <div id="header">
        <h2>CORE OCCUPANCY OF RUNNING MULTICORE PILOTS AT CMS T2s for the last '$int' hours, updated at '$(date -u)'<br>
...
<a href="http://submit-3.t2.ucsd.edu/CSstoragePath/aperez/HTML/T2s/multicore_occupancy_t2s_24h.html">24h</a>
<a href="http://submit-3.t2.ucsd.edu/CSstoragePath/aperez/HTML/T2s/multicore_occupancy_t2s_168h.html">1week</a>
<a href="http://submit-3.t2.ucsd.edu/CSstoragePath/aperez/HTML/T2s/longmulticore_occupancy_t2s_720h.html">1month</a>
<br>
