...
tail -n $n_lines $OUTDIR/jobs_running_global |awk -v var="$ratio" 'NR % var == 0'>$WORKDIR/status/input_jobs_running_global$int
while read -r line; do
        time=$(echo $line |awk '{print $1}')
        let timemil=1000*$time
...
tail -n $n_lines $OUTDIR/jobs_running_T0AndGlobalPool |awk -v var="$ratio" 'NR % var == 0'>$WORKDIR/status/input_jobstier_running_global$int
while read -r line; do
        time=$(echo $line |awk '{print $1}')
        let timemil=1000*$time
...
<body>
	...
	<br><a href="'$WEBPATH'T3s/multicore_usage_T3s_24h.html" target="blank">T3 mcore pilots</a> and
        <a href="'$WEBPATH'JobInfo/jobstatus_T3_24h.html" target="blank"> jobs</a>
