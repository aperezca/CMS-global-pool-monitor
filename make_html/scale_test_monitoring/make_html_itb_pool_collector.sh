...
OUT=$HTMLDIR/$long"itb_pool_collector_"$int"h.html"
#--------------------
echo '<html>
<head>
<title>CMS ITB pool collector metrics</title>
<!--Load the AJAX API-->
...
#-------------
tail -n $n_lines $OUTDIR/collector_itb_pool|awk -v var="$ratio" 'NR % var == 0' |sort >$WORKDIR/status/input_itb_collector$int
...
done <$WORKDIR/status/input_itb_collector$int
...
<body>
    ...
    <div id="header">
        ...
        <a href="'$WEBPATH'itb_pool_collector_24h.html">24h</a>
        <a href="'$WEBPATH'itb_pool_collector_168h.html">1week</a>
        <a href="'$WEBPATH'longitb_pool_collector_720h.html">1month</a>
        <br><br>
        </h2>
    </div>
