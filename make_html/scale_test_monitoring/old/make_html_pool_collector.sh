...
OUT=$OUTDIR"/HTML/"$long"pool_collector_"$int"h.html"
echo '<html>
<head>
<title>CMS test pool collector metrics</title>
<!--Load the AJAX API-->
...
#-------------
tail -n $n_lines $OUTDIR/out/collector_itb_pool|awk -v var="$ratio" 'NR % var == 0' |sort >$WORKDIR/status/input_collector$int
...
done <$WORKDIR/status/input_collector$int
...
<body>
    ...
    <div id="header">
        <h2>CMS test POOL collector metrics monitor for the last '$int' hours, updated at '$(date -u)'<br>
        <a href="http://submit-3.t2.ucsd.edu/CSstoragePath/aperez/scale_test_monitoring/HTML/pool_collector_24h.html">24h</a>
        <a href="http://submit-3.t2.ucsd.edu/CSstoragePath/aperez/scale_test_monitoring/HTML/pool_collector_168h.html">1week</a>
        <a href="http://submit-3.t2.ucsd.edu/CSstoragePath/aperez/scale_test_monitoring/HTML/longpool_collector_720h.html">1month</a>
        <br><br>
        </h2>
    </div>
