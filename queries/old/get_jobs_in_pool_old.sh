...
cat /home/aperez/status/all_jobs.txt |grep tier0 >/home/aperez/status/all_jobs_tier0.txt
cat /home/aperez/status/all_jobs.txt |grep -v crab |grep -v prod |grep -v tier0 >/home/aperez/status/all_jobs_other.txt
...
echo $date_all $jobs_run_tier0 $jobs_idle_tier0 $jobs_held_tier0 >>/crabprod/CSstoragePath/aperez/out/jobs_size_tier0
echo $date_all $jobs_run_other $jobs_idle_other $jobs_held_other >>/crabprod/CSstoragePath/aperez/out/jobs_size_other

echo $date_all $autoclusters_prod $autoclusters_crab $autoclusters_tier0 $autoclusters_other >>/crabprod/CSstoragePath/aperez/out/autoclusters

# -----------------------------
# count also number of autoclusters from queued jobs:
autoclusters_prod_queued=$(cat /crabprod/CSstoragePath/aperez/HTML/JobInfo/globalpool_all_queued_jobs.txt |awk '{print $9, $10}' | grep production |sort |uniq |wc -l)
autoclusters_crab_queued=$(cat /crabprod/CSstoragePath/aperez/HTML/JobInfo/globalpool_all_queued_jobs.txt |awk '{print $9, $10}' | grep analysis |sort |uniq |wc -l)
autoclusters_tier0_queued=$(cat /crabprod/CSstoragePath/aperez/HTML/JobInfo/globalpool_all_queued_jobs.txt |awk '{print $9, $10}' | grep tier0 |sort |uniq |wc -l)


echo $date_all $autoclusters_prod_queued $autoclusters_crab_queued $autoclusters_tier0_queued >>/crabprod/CSstoragePath/aperez/out/autoclusters_queued
