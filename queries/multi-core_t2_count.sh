...
for site in `cat /home/aperez/entries/T2_sites`; do
	...
	if [ $site == "T1_RU_JINR" ]; then
		...
		#let pilot_$i=$(cat glideins_$site| grep Partitionable |awk '{print $3}' |grep $used_cores| wc -l); done
	        echo $date $pilot_0 $pilot_1 $pilot_2 $pilot_3 $pilot_4 $pilot_5 $pilot_6 $pilot_7 $pilot_8 $pilot_9 $pilot_10 $pilot_11 $pilot_12 >>/home/aperez/out/occup_$site
	else	
		for i in {0..8}; do 
		let used_cores=8-$i
...
date_all=`date -u +%s`
echo $date_all $n_pilots_all $n_cores_all $n_cores_tot_all $n_cores_busy_all $n_cores_idle_all $cores_in_child_preempting_all >>/home/aperez/out/count_All_T2s
