...
for site in `cat "$WORKDIR/entries/"$list"_sites"`; do
	...
	echo $date_s $n_pilots $n_cores_part $n_cores_tot $n_cores_busy $n_cores_idle $cores_in_child_preempting >>$OUTDIR/out/count_$site
	...
	echo $date_s $string>>$OUTDIR/out/occup_$site
	echo ""
	#echo $date_s $pilot_0 $pilot_1 $pilot_2 $pilot_3 $pilot_4 $pilot_5 $pilot_6 $pilot_7 $pilot_8 >>$OUTDIR/out/occup_$site
done

#echo $list
#echo $date_s $n_pilots_all $n_cores_all $n_cores_tot_all $n_cores_busy_all $n_cores_idle_all $cores_in_child_preempting_all
echo $date_s $n_pilots_all $n_cores_all $n_cores_tot_all $n_cores_busy_all $n_cores_idle_all $cores_in_child_preempting_all >>$OUTDIR/out/count_All_"$list"s
