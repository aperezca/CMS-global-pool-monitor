...
for i in 'prod' 'crab' 'tier0' 'other'; do
	...
	let jobcores_run_$i=0
		...
		echo $schedd
                condor_q -pool $collector -name $schedd -af JobStatus RequestCPUs AutoClusterId |sort |uniq -c >$WORKDIR/status/stats_jobcores_$schedd$date_all
                schedd_jobs_run=0
                schedd_jobs_idle=0
                schedd_cores_run=0
                schedd_cores_idle=0
                ...
                done<$WORKDIR/status/stats_jobcores_$schedd$date_all
                rm $WORKDIR/status/stats_jobcores_$schedd$date_all
