...
condor_status -pool $collector -const '(SlotType=?="Static") && ((IOslots=?=undefined) || (IOslots != 1))' -af GLIDEIN_CMSSite SlotType TotalSlotCpus GLIDEIN_MaxMemMBs |sort |uniq -c >$WORKDIR/status/all_static_glideins.txt
...
echo "## -------------------------------------------------------------------------" >>$OUT
cat $WORKDIR/status/all_partitionable_glideins.txt >>$OUT
echo "## -------------------------------------------------------------------------" >>$OUT
