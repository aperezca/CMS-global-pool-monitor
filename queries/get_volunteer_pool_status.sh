...
date_all=`date -u +%s`
echo $date_all $size_part $size_stat >>$OUTDIRVOL/pool_size
#----------------

now=$(date -u)
echo "## -------------------------------------------------------------------------"  >$HTMLDIR/volunteer_pool_pilot_info.txt
echo "## INFO ON CMS VOLUNTEER (CMS@Home) POOL PILOTS UPDATED AT" $now              >>$HTMLDIR/volunteer_pool_pilot_info.txt
echo "## -------------------------------------------------------------------------" >>$HTMLDIR/volunteer_pool_pilot_info.txt
echo "## FRESH GLIDEINS: #_slots GLIDEIN_CMSSite GLIDEIN_Entry_Name SlotType TotalSlotCPUs CPUs TotalRepackSlots TotalIOSlots RepackSlots IOSlots Memory State Activity">>$HTMLDIR/volunteer_pool_pilot_info.txt
echo "## -------------------------------------------------------------------------" >>$HTMLDIR/volunteer_pool_pilot_info.txt
cat $WORKDIR/status/Volunteer_pool/cores_fresh_glideins.txt                              >>$HTMLDIR/volunteer_pool_pilot_info.txt
echo "## -------------------------------------------------------------------------" >>$HTMLDIR/volunteer_pool_pilot_info.txt
#echo "## RETIRING GLIDEINS: #_slots GLIDEIN_CMSSite GLIDEIN_Entry_Name SlotType TotalSlotCPUs CPUs TotalRepackSlots TotalIOSlots RepackSlots IOSlots Memory State Activity" >>$HTMLDIR/volunteer_pool_pilot_info.txt
#echo "## -------------------------------------------------------------------------" >>$HTMLDIR/volunteer_pool_pilot_info.txt
#cat $WORKDIR/status/Volunteer_pool/cores_retiring_glideins.txt                                     >>$HTMLDIR/volunteer_pool_pilot_info.txt
#echo "## -------------------------------------------------------------------------" >>$HTMLDIR/volunteer_pool_pilot_info.txt
...
echo $date_s $slots_fresh_1 $slots_fresh_2 $slots_fresh_3 $slots_fresh_4 $slots_fresh_5 $slots_fresh_6 $slots_fresh_7 $slots_fresh_8 >>$OUTDIRVOL/pool_partition_fresh

# Count number of dyn slots in the pool
echo $date_s $dyn_slots >>$OUTDIRVOL/pool_dynslots
