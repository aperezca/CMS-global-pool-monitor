...
#Pool to query
collector=$(/home/aperez/collector_t0.sh)
...
for site in `cat "/home/aperez/entries/"$list"_sites"`; do
	...
	echo $date_s $n_pilots $n_cores_part $n_cores_tot $n_cores_busy $n_cores_idle $cores_in_child_preempting >>/crabprod/CSstoragePath/aperez/out/T0/count_$site
	...
	echo $date_s $string>>/crabprod/CSstoragePath/aperez/out/T0/occup_$site
	echo $date_s $string
	echo ""
done
