...
for site in `cat "$WORKDIR/entries/"$list"_sites"`; do
	...
	echo $date_s $n_pilots $n_cores_part $n_cores_tot $n_cores_busy $n_cores_idle $cores_in_child_preempting >>$OUTDIR/out/T0/count_$site
	...
	echo $date_s $string>>$OUTDIR/out/T0/occup_$site
	echo $date_s $string
	echo ""
done
...
echo $date_s $n_pilots_all $n_cores_all $n_cores_tot_all $n_cores_busy_all $n_cores_idle_all $cores_in_child_preempting_all >>$OUTDIR/out/T0/count_All_"$list"s
