...
for site in `cat /home/aperez/entries/T0_sites`; do
	...
	-format '%s\ ' CPUs \
	-format '%s\ ' State \
	-format '%s\n' Activity > /home/aperez/status/T0/glideins_$site
	...
	cores_in_parent=0
	for i in `cat /home/aperez/status/T0/glideins_$site |grep Partitionable |awk '{print $3}'`; do let cores_in_parent+=i; done
	cores_in_child_busy=0
	for i in `cat /home/aperez/status/T0/glideins_$site |grep Dynamic |grep "Claimed Busy" | awk '{print $3}'`; do let cores_in_child_busy+=i; done
	cores_in_child_idle=0
        for i in `cat /home/aperez/status/T0/glideins_$site |grep Dynamic |grep "Unclaimed Idle" | awk '{print $3}'`; do let cores_in_child_idle+=i; done
	cores_in_child_preempting=0
        for i in `cat /home/aperez/status/T0/glideins_$site |grep Dynamic |grep "Preempting Killing" | awk '{print $3}'`; do let cores_in_child_preempting+=i; done
	...
	echo $date $n_pilots $n_cores $n_cores_tot $n_cores_busy $n_cores_idle $cores_in_child_preempting >>/crabprod/CSstoragePath/aperez/out/T0/count_$site
	...
	echo $date $pilot_0 $pilot_1 $pilot_2 $pilot_3 $pilot_4 $pilot_5 $pilot_6 $pilot_7 $pilot_8 >>/crabprod/CSstoragePath/aperez/out/T0/occup_$site
done
date_all=`date -u +%s`
echo $date_all $n_pilots_all $n_cores_all $n_cores_tot_all $n_cores_busy_all $n_cores_idle_all $cores_in_child_preempting_all >>/crabprod/CSstoragePath/aperez/out/T0/count_All_T0s
