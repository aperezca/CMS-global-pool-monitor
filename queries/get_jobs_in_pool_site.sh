...
for schedd in $(cat $WORKDIR/status/all_jobs | awk '{print $1}'); do
	echo $schedd 
	condor_q -pool $collector -name $schedd -constraint '(JobUniverse == 5) && (JobStatus == 2)' -af MATCH_GLIDEIN_CMSSite WMAgent_RequestName CMS_JobType CRAB_ReqName RequestCPUs RequestMemory JobPrio AcctGroup |sort |uniq -c >>$OUTDIR/HTML/JobInfo/globalpool_running_jobs_$date_all.txt 
done
...
for list in T1 T2 T2_singlecore T0; do
	...
	for site in `cat $WORKDIR"/entries/"$list"_sites"`; do
		...
		mv $OUTDIR/HTML/JobInfo/running_jobs_$site$date_all.txt $OUTDIR/HTML/JobInfo/running_jobs_$site.txt
...
mv $OUTDIR/HTML/JobInfo/running_jobs_at_T1$date_all.txt $OUTDIR/HTML/JobInfo/running_jobs_at_T1.txt
mv $OUTDIR/HTML/JobInfo/running_jobs_at_T2$date_all.txt $OUTDIR/HTML/JobInfo/running_jobs_at_T2.txt
mv $OUTDIR/HTML/JobInfo/running_jobs_at_T2_singlecore$date_all.txt $OUTDIR/HTML/JobInfo/running_jobs_at_T2_singlecore.txt
mv $OUTDIR/HTML/JobInfo/running_jobs_at_T0$date_all.txt $OUTDIR/HTML/JobInfo/running_jobs_at_T0.txt
...
cat $OUTDIR/HTML/JobInfo/globalpool_running_jobs_$date_all.txt |sort -k2 >>$OUTDIR/HTML/JobInfo/globalpool_running_jobs.txt
